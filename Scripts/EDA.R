...
# Create the bar plot
ggplot(top_10_counts_end, aes(x = reorder(end_station_name, -n), y = n)) +
  geom_bar(stat = "identity", fill = "gold") +
  labs(title = "Top 10 Locations Where People Bike To", x = "End Station", y = "Number of Rides (in ten thousands)")+
  theme(axis.text.x=element_text(size=rel(0.8), angle=90))
